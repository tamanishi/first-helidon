...
```
java -jar target/first-helidon.jar
```
...
```
docker build -t first-helidon .
```

## Start the application with Docker

```
docker run --rm -p 8080:8080 first-helidon:latest
```
...
kubectl get service first-helidon  # Get service info
```
...
```
docker build -t first-helidon-native -f Dockerfile.native .
```

Start the application:

```
docker run --rm -p 8080:8080 first-helidon-native:latest
```